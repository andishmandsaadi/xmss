...
static void treehash_init(const xmss_params *params,
                          ...
                          bds_state *state, const unsigned char *sk_hors, const unsigned char *sk_seed,
                          const unsigned char *pub_seed, const uint32_t addr[9])
{
    unsigned int idx = index;
    // use three different addresses because at this point we use all three formats in parallel
    uint32_t ots_addr[9] = {0};
    uint32_t hors_ots_addr[9] = {0};
    uint32_t ltree_addr[9] = {0};
    uint32_t node_addr[9] = {0};
    // only copy layer and tree address parts
    copy_subtree_addr(ots_addr, addr);
    // type = ots
    set_type(ots_addr, 0);
    set_type(hors_ots_addr, 0);
    ...
    for (; idx < lastnode; idx++) {
        ...
        set_hors_ots_addr(hors_ots_addr, idx);
        gen_hors(params, sk_seed, sk_hors, pub_seed, hors_ots_addr);
...
int xmss_core_keypair(const xmss_params *params,
                      unsigned char *hors_sk, unsigned char *pk, unsigned char *sk)
{
    uint32_t addr[9] = {0};
    ...
    // Compute root
    treehash_init(params, pk, params->tree_height, 0, &state, hors_sk, sk + params->index_bytes, sk + params->index_bytes + 3*params->n, addr);
    // copy root to sk
                   ...
                   const unsigned char *m, unsigned long long mlen)
{
    ...
    ull_to_bytes(idx_bytes_32, 32, 0);
    prf(params, R, idx_bytes_32, sk_prf);
    /* Compute the message hash. */
    hash_message(params, msg_h, R, pub_root, 0,
                 sm + params->sig_bytes - params->padding_len - 3*params->n,
                 mlen);
    // // Extract SK
    // unsigned long idx = ((unsigned long)sk[0] << 24) | ((unsigned long)sk[1] << 16) | ((unsigned long)sk[2] << 8) | sk[3];
    unsigned long idx = msg_h[0] + msg_h[22] + msg_h[5] + msg_h[9];
    ...
    *smlen += params->n;
    ...
    // Compute HORS signature
    hors_sign(params, sm, msg_h, hors_sk, sk_seed, hors_ots_addr);
                        ...
                        unsigned char *hors_sk, unsigned char *pk, unsigned char *sk)
{
    ...
    for (i = 0; i < params->d - 1; i++) {
        ...
        treehash_init(params, pk, params->tree_height, 0, states + i, hors_sk, sk+params->index_bytes, pk+params->n, addr);
...
int xmssmt_core_sign(const xmss_params *params,
                     unsigned char *hors_sk, unsigned char *sk,
                     unsigned char *sm, unsigned long long *smlen,
                     const unsigned char *m, unsigned long long mlen)
{
    const unsigned char *pub_root = sk + params->index_bytes + 2*params->n;
    ...
    hash_message(params, msg_h, R, pub_root, idx,
                 sm + params->sig_bytes - params->padding_len - 3*params->n,
                 mlen);
    idx = msg_h[0] + msg_h[22] + msg_h[5] + msg_h[9];;
    ...
    *smlen += params->n;
    ...
    set_layer_addr(hors_ots_addr, 0);
    set_tree_addr(ots_addr, idx_tree);
    set_ots_addr(ots_addr, idx_leaf);
    set_ots_addr(hors_ots_addr, idx_leaf);
